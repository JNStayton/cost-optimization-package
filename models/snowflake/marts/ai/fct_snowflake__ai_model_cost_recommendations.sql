    ...
    case
        when recommendation_key = 'model_downgrade'
            then 'Average output of ' || avg_output_tokens || ' tokens/query suggests simple responses. '
                ...
                || coalesce(function_name, 'unknown') || ' workload. Projected annual cost: $' || projected_annual_cost_usd || '.'
        when recommendation_key = 'prompt_bloat'
            then 'Average input of ' || avg_input_tokens || ' tokens/query (' || coalesce(function_name, 'unknown') || '). '
        ...
        when recommendation_key = 'batch_opportunity'
            then avg_daily_calls || ' calls/day to ' || coalesce(model_name, 'unknown') || ' (' || coalesce(function_name, 'unknown') || '). '
        ...
        else 'Usage of ' || coalesce(model_name, 'unknown') || ' (' || coalesce(function_name, 'unknown') || ') within normal parameters. '
