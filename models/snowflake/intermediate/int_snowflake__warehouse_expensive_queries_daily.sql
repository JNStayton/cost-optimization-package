...
{#--
  ...
  Credit attribution (automatic fallback):
    - Primary: QUERY_ATTRIBUTION_HISTORY.credits_attributed_compute (exact per-query)
    - Fallback: when attribution is NULL (short queries <= ~100ms, or queries before
      Aug 2024), credits are estimated from WAREHOUSE_METERING_HISTORY prorated by
      elapsed time share within the warehouse-hour.
    - The credits_from_attribution column flags which method was used per row.
...
query_credits as (
    select
        ...
        dq.dbt_node_id,
        -- Primary: exact attribution. Fallback: elapsed-time proration.
        coalesce(
            qah.credits_attributed_compute,
            coalesce(mh.credits_in_hour, 0)
                * (dq.total_elapsed_time_ms / nullif(wht.total_elapsed_ms_in_hour, 0))
        )                                                           as credits_attributed_compute,
...
select
    ...
    -- Top user/role by run count for this query pattern
    mode(user_name)                                                 as top_user_name,
    mode(role_name)                                                 as top_role_name,
