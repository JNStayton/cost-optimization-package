...
chain_context as (
    ...
    left join {{ ref('int_databricks__view_chains') }} as ch
        on ch.model_fqn = ss.table_fqn
),

view_chain_fqns as (
    -- pre-flatten the per-view downstream table FQN array so we can join to
    -- query history once per (view, downstream_table) pair
    select
        vc.model_fqn,
        fqn_flat.fqn as downstream_table_fqn
    from (
        select distinct model_fqn, downstream_table_fqns
        from {{ ref('int_databricks__view_chains') }}
        where size(downstream_table_fqns) > 0
    ) as vc
    lateral view explode(vc.downstream_table_fqns) fqn_flat as fqn
),

downstream_build_stats as (
    -- avg build time of downstream tables per view; matches on the unqualified
    -- table name (third segment of the FQN)
    select
        vcf.model_fqn,
        avg(qh.execution_time_ms) / 1000.0 as downstream_build_time_s
    from view_chain_fqns as vcf
    join {{ ref('int_databricks__query_history') }} as qh
        on qh.query_text ilike '%' || split_part(vcf.downstream_table_fqn, '.', 3) || '%'
       and qh.statement_type in ('CREATE_TABLE_AS', 'INSERT', 'MERGE')
       and qh.query_start_time >= current_timestamp() - INTERVAL 1 MONTH
    group by vcf.model_fqn
),
