{#--
  Dispatcher for find_expensive_dbt_queries. Platform implementations:
    - snowflake__find_expensive_dbt_queries   (macros/snowflake/optimizations/snowflake__find_expensive_dbt_queries.sql)
  Add a <platform>__find_expensive_dbt_queries with the same arguments to support another platform.
--#}
{% macro find_expensive_dbt_queries(lookback_days=7, top_n=20, min_total_credits=0.1, credit_rate_usd=2, high_cost_threshold_usd=10000, dbt_project_only=true) %}
...
{% macro default__find_expensive_dbt_queries(lookback_days=7, top_n=20, min_total_credits=0.1, credit_rate_usd=2, high_cost_threshold_usd=10000, dbt_project_only=true) %}
  {% if execute %}
