{% macro redshift__probe_unique_key_candidates() %}

  {#--
    Post-hook for fct_redshift__incremental_config_recommendations.

    Redshift port of the Snowflake probe_unique_key_candidates macro.

    For each candidate table, probes unique_key_candidates using
    APPROXIMATE COUNT(DISTINCT ...) to find the first column whose cardinality
    is approximately equal to the row count (>= 95% threshold). All candidate
    columns for a table are probed in a single scan.
  ...
  {% if execute and target.type == 'redshift' %}
    ...
    {# Fetch candidates; serialize the SUPER array to JSON string for Jinja parsing. #}
    {% set candidates_sql %}
      select
        table_fqn,
        best_unique_key,
        json_serialize(unique_key_candidates) as candidates_json,
        incremental_strategy,
        lower(suggested_filter_column)        as suggested_filter_column
      from {{ this }}
      where unique_key_candidates is not null
        and json_serialize(unique_key_candidates) != '[]'
    {% endset %}
    ...
    {% if candidates and candidates.rows | length > 0 %}
      ...
      {% for row in candidates %}
        ...
        {% set candidates_json = row['candidates_json'] %}
        ...
        {# Parse JSON array string ["col1","col2"] into a list of strings.
           Strip outer brackets, split on '","', strip remaining quotes. #}
        {% set inner = candidates_json[1:-1] %}
        {% set raw_parts = inner.split('","') %}
        {% set candidate_cols = [] %}
        {% for p in raw_parts %}
          {% set cleaned = p | replace('"', '') | trim %}
          {% if cleaned %}
            {% do candidate_cols.append(cleaned) %}
          {% endif %}
        {% endfor %}

        {{ log("probe_unique_key_candidates: probing " ~ (candidate_cols | length) ~ " candidate(s) for " ~ table_fqn, info=true) }}

        {# Scan the table once — one APPROXIMATE COUNT(DISTINCT ...) per candidate column #}
        {% set probe_sql %}
          select
            count(*) as total_rows
            {% for col in candidate_cols %}
              , approximate count(distinct "{{ col }}") as col_{{ loop.index }}_distinct
            {% endfor %}
          from {{ table_fqn }}
        {% endset %}
